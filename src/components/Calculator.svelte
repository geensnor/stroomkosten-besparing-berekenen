<script lang="ts">
  ...
  let priceWh: number = 0.4;
  let priceInvestment: number = 100;
  let powerYearBefore: number;
  let powerYearAfter: number;
  let co2Intensity: number = 330;
</script>
...
<div class="grid grid-cols-4 gap-4">
  ...
  <div class="col-span-4 md:col-span-2">
    <Card title="Wat is dit? 🤔"
      ><p>
        Als je stroomkosten wilt besparen, is het handig als je het eerst even
        doorrekent. Met deze tool kun je berekenen hoe lang het duurt voordat je
        je investering hebt terugverdiend. Als je niets investeert, kun je
        uitrekenen hoeveel je per jaar bespaart als je minder stroom gaat
        gebruiken.
      </p>
      <p class="mt-4 text-xs">
        Voor de uitstoot wordt uitgegaan van {co2Intensity} gram CO₂ per kWh. Bron:
        <a
